...
def loadDocs():
    name_documents = []
    with open("project_root/backend/NameData.txt", "r", encoding="utf-8", errors="replace") as f:
...
def vectorizeDB(name_documents):
...
def buildQueryEngine(name_index):
    # Create a query engine from the index
    name_query_engine = name_index.as_query_engine(return_source=True, similarity_top_k=10)
    return name_query_engine

documents = loadDocs()
name_query_engine = buildQueryEngine(vectorizeDB(documents))
